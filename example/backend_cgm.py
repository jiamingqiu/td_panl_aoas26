...
def parse_simu_args(*args, **kwargs):
    ...
    dat_config.add_argument(
        '--window_hours', type=float, default=1,
        help='duration (in hour) of monitoring window'
    )
    dat_config.add_argument(
        '--how_soon_hours', type=float, default=0.5,
        help=(
    ...
    dat_config.add_argument(
        '--event_free_window', type=str, default='y', choices=['n', 'y'],
        help='whether monitoring window should be event free.'
    )
    dat_config.add_argument(
        '--event_later_than', type=float, default=0.75, 
        help=(
    ...
    dat_config.add_argument(
        '--n_samples', type=int, default=5120,
        help=(
    ...
    dat_config.add_argument(
        '--event_ratio', type=float, default=0.3,
        help='Proportion of positive cases.'
    )
...
def gen_data(args, use_device = None):
    ...
    ls_dat = get_cgm(
        args.n_samples, args.event_ratio,
        args.window_hours, args.how_soon_hours,
        event_free_window, args.event_later_than
    ...
    dict_dat = {}
    dict_loader = {}
    for partition, ls_this_partition in zip(
        ["train", "validate", "test"], ls_dat
    ):
        dict_this_partition = {
